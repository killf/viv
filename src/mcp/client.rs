...
impl<T: Transport> McpClient<T> {
    ...
    pub async fn list_tools(&mut self) -> crate::Result<Vec<McpTool>> {
        ...
        loop {
            ...
            if let Some(tools_json) = result.get("tools") {
                let tools = McpTool::parse_list(tools_json)?;
    ...
    pub async fn list_resources(&mut self) -> crate::Result<Vec<McpResource>> {
        ...
        loop {
            ...
            if let Some(resources_json) = result.get("resources") {
                let resources = McpResource::parse_list(resources_json)?;
    ...
    pub async fn read_resource(&mut self, uri: &str) -> crate::Result<ResourceContent> {
        ...
        // The result contains a "contents" array; return the first item
        if let Some(contents) = result.get("contents").and_then(|v| v.as_array())
            && let Some(first) = contents.first()
        {
            return ResourceContent::from_json(first);
        }

        Err(Error::Mcp {
            server: String::new(),
            message: "Empty resource content".to_string(),
        })
    }
    ...
    pub async fn list_prompts(&mut self) -> crate::Result<Vec<McpPrompt>> {
        ...
        loop {
            ...
            if let Some(prompts_json) = result.get("prompts") {
                let prompts = McpPrompt::parse_list(prompts_json)?;
