...
pub struct UnixTimer { fd: i32 }

impl UnixTimer {
    pub fn new(duration: Duration) -> crate::Result<Self> {
        let fd = unsafe { timerfd_create(CLOCK_MONOTONIC, TFD_NONBLOCK) };
        if fd < 0 { return Err(crate::Error::Io(std::io::Error::last_os_error())); }
        let spec = Itimerspec {
            it_interval: Timespec { tv_sec: 0, tv_nsec: 0 },
            it_value: Timespec { tv_sec: duration.as_secs() as i64, tv_nsec: duration.subsec_nanos() as i64 },
        };
        let ret = unsafe { timerfd_settime(fd, 0, &spec, std::ptr::null_mut()) };
        if ret != 0 { unsafe { close(fd) }; return Err(crate::Error::Io(std::io::Error::last_os_error())); }
        Ok(UnixTimer { fd })
    }

    pub fn handle(&self) -> RawHandle { self.fd }
