...
use crate::memory::index::{EntryKind, MemoryEntry, MemoryIndex};
...
pub async fn evolve_from_session(
    ...
    llm.stream_agent_async(&system, &req_msgs, "", ModelTier::Medium, |t| {
        response.push_str(t)
    })
    .await?;
...
fn parse_learnings(response: &str) -> Vec<Learning> {
    ...
    let json = match JsonValue::parse(json_str) {
        Ok(j) => j,
        Err(_) => return vec![],
    };
...
fn extract_tags_from_summary(summary: &str) -> Vec<String> {
    const STOP_WORDS: &[&str] = &[
        "the", "a", "an", "in", "on", "at", "to", "for", "of", "and", "or", "with", "this", "that",
        "was", "were", "is", "are", "it", "its", "been", "have", "has", "had", "from", "by", "as",
        "be", "not", "but",
    ];
    summary
