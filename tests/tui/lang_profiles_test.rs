...
fn select_rust_profile() {
    ...
    assert!(p.keywords.contains(&"fn"), "rust profile should have 'fn' keyword");
    assert!(p.lifetime_prefix, "rust profile should have lifetime_prefix = true");
}
...
fn select_python_profile() {
    ...
    assert!(p.keywords.contains(&"def"), "python profile should have 'def' keyword");
    assert!(p.triple_quote, "python profile should have triple_quote = true");
    assert_eq!(p.line_comments, &["#"], "python profile should use '#' for line comments");
}
...
fn select_js_profile() {
    ...
    assert!(p.template_literal, "js profile should have template_literal = true");
}
...
fn select_go_profile() {
    ...
    assert_eq!(p.name, "go");
    assert!(p.type_starts_upper, "go profile should have type_starts_upper = true");
}
...
fn select_shell_profile() {
    ...
    assert!(p.keywords.contains(&"fi"), "shell profile should have 'fi' keyword");
}
...
fn select_json_profile() {
    ...
    assert!(p.line_comments.is_empty(), "json profile should have no line comments");
}
