...
fn welcome_renders_logo() {
    ...
    assert!(
        row1.contains('_'),
        "logo row 1 should contain '_': got '{row1}'"
    );
}
...
fn welcome_renders_model_info() {
    ...
    assert!(
        full.contains("Model"),
        "should contain Model label: got '{full}'"
    );
}
...
fn welcome_renders_placeholder_when_no_model() {
    ...
    assert!(
        full.contains("..."),
        "should show '...' when model unknown: got '{full}'"
    );
}
...
fn welcome_logo_uses_claude_color() {
    ...
    let mut buf = Buffer::empty(area);
    widget.render(area, &mut buf);
    let logo_cell = (0..20).map(|x| buf.get(x, 1)).find(|c| c.ch != ' ');
