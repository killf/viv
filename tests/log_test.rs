...
fn module_name_from_deep_path() {
    assert_eq!(viv::log::extract_module("src/core/runtime/mod.rs"), "core.runtime");
}
...
fn record_format_contains_fields() {
    ...
    let rec = Record::new(Level::Info, "test".to_string(), "src/test.rs", 42, "hello world".into());
    let s = rec.to_string();
...
fn record_format_time_is_rfc3339() {
    use viv::log::{Level, Record};
    let rec = Record::new(Level::Info, "test".to_string(), "src/test.rs", 42, "msg".into());
    let s = rec.to_string();
