...
#[test]
fn log_file_written() {
    run_with_lock("/tmp/viv_it_a.log", Level::Debug, || {
        viv::info!("msg1 {}", 1);
        viv::debug!("msg2 {}", 2);
        viv::error!("msg3 {}", 3);
    });
    ...
    let mut f = fs::File::open("/tmp/viv_it_a.log").expect("open log");
...
#[test]
fn log_level_filter() {
    run_with_lock("/tmp/viv_it_b.log", Level::Warn, || {
        viv::info!("msg_x");
        viv::error!("msg_y");
    });

    let mut c = String::new();
    let mut f = fs::File::open("/tmp/viv_it_b.log").expect("open log");
    f.read_to_string(&mut c).expect("read log");
