...
fn second_diff_returns_empty_after_sync() {
    ...
    assert!(
        bytes.is_empty(),
        "expected empty second diff, got {} bytes",
        bytes.len()
    );
}
...
fn clear_back_then_modify_shows_change() {
    ...
    assert!(
        !bytes.is_empty(),
        "expected diff after clear_back changed a cell"
    );
}
...
fn diff_plain_cell_no_style_sequences() {
    ...
    let no_color =
        !text.contains("\x1b[30m") && !text.contains("\x1b[31m") && !text.contains("\x1b[32m");
